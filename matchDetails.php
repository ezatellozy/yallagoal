<?php include 'layouts/header.php'; ?>
<section>
    <div class="container-fluid match-details">
        <div class="container">
            <div class="row mt-3 matchCenterTable" data-id="3017089">
                <div class="col">
                    <div class="row quickView">
                        <div class="col teamA text-start">
                            <div class="mb-2">
                                <a href="/team/11867/empoli" title="إمبولي">
                                    <img src="https://img.btolat.com/teamslogo/11867.png" class="teamImage"
                                        alt="إمبولي">
                                    <span class="teamNameX text-white"> إمبولي </span>
                                </a>
                            </div>
                            <div class=" getGoalsTeamA">
                                <div class="goals">
                                    <div class="getGoal">
                                        <span><img src="./assets/images/goalBall.png" alt="هدف"></span>
                                        <span class="goalTime clr2">31'</span>
                                        <span class="goalPlayer">باتريك كوتروني</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col text-center">
                            <div class="row">
                                <div class="col-3">
                                    <h2 class="font-weight-bold teamAScore">1 </h2>
                                </div>
                                <div class="col-6">
                                    <h3 class="clr2 font-weight-bold matchStatusAr">إنتهت</h3>
                                </div>
                                <div class="col-3">
                                    <h2 class="font-weight-bold teamBScore">1 </h2>
                                </div>
                            </div>
                        </div>
                        <div class="col teamB text-end">
                            <div class="mb-2">
                                <a href="/team/12002/salernitana" title="ساليرنيتانا">
                                    <img src="https://img.btolat.com/teamslogo/12002.png" class="teamImage"
                                        alt="ساليرنيتانا">
                                    <span class="teamNameX text-white"> ساليرنيتانا </span>
                                </a>
                            </div>
                            <div class="getGoalsTeamB">
                                <div class="goals">
                                    <div class="getGoal">
                                        <span><img src="./assets/images/goalBall.png" alt="هدف"></span>
                                        <span class="goalTime clr2">83'</span>
                                        <span class="goalPlayer">دييجو بيروتي</span>
                                    </div>
                                    <div class="getGoal">
                                        <span><img src="./assets/images/goalBall.png" alt="هدف"></span>
                                        <span class="goalTime clr2">76'</span>
                                        <span class="goalPlayer">فيدريكو بونازولي</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row  position-relative">
                        <div class="col matchTimer">
                            <div class="timeNow bg2">
                                <span class="bg2"></span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row bg1 lastNewsMatchCenter">
                <div class="bobo">
                    <div class="lam">
                        <img src="./assets/images/event.png" alt="">
                        <span>السبت , 14 مايو 2022</span>
                    </div>
                    <div class="lam">
                        <img src="./assets/images/stopwatch.png" alt="">
                        <span>03:00 م</span>
                    </div>
                </div>
            </div>
            <div class="container-fluid">
                <div class="row page-content mt-3 pages position-relative">
                    <div class="col-xl-8 right-side allNews overflow-auto">
                        <div class="container">

                            <div class="rightBoxes" style="position: inherit; top: inherit;">
                                <div class="row">
                                    <div class="col rshandler">
                                        <div class="row tabsLinks shadow-sm bg-white">
                                            <div class="col"><a class="active" href="matchDetails.php">معلومات</a>
                                            </div>
                                            <div class="col"><a href="matchDetailsLineup.php">التشكيل</a></div>
                                            <div class="col"><a href="matchDetailsEvents.php">احداث</a></div>
                                            <div class="col"><a href="matchDetailsStatistics.php">احصائيات</a></div>
                                            <div class="col"><a href="matchDetailsNews.php">اخبار و فيديو</a></div>
                                        </div>
                                        <div class="row tabBody">
                                            <div class="col">
                                                <div class="col">
                                                    <div>
                                                        <h1 class="tabTitle mx-auto">
                                                            <span>تاريخ مواجهات إمبولي و ساليرنيتانا</span>
                                                        </h1>
                                                        <!-- End tabTitle -->
                                                        <div class="card">
                                                            <div class="row">
                                                                <div class="col fastHis text-right">
                                                                    <img src="https://img.btolat.com/teamslogo/11867.png"
                                                                        class="imgHis" alt="إمبولي">
                                                                    <span>إمبولي</span>
                                                                </div>
                                                                <div class="col fastHis text-center">
                                                                    17 مباراة
                                                                </div>
                                                                <div class="col fastHis text-left">
                                                                    <img src="https://img.btolat.com/teamslogo/12002.png"
                                                                        class="imgHis" alt="ساليرنيتانا">
                                                                    <span>ساليرنيتانا</span>
                                                                </div>
                                                            </div>
                                                            <div class="row historyChart">
                                                                <div class="historyChartTeamAWin"
                                                                    style="width: 58.8235%;">
                                                                    <span></span>
                                                                    <p>
                                                                        فوز
                                                                        <span>10</span>
                                                                    </p>
                                                                </div>
                                                                <div class="historyChartDraw" style="width: 17.6471%;">
                                                                    <span></span>
                                                                    <p>
                                                                        تعادل
                                                                        <span>3</span>
                                                                    </p>
                                                                </div>
                                                                <div class="historyChartTeamBWin"
                                                                    style="width: 23.5294%;">
                                                                    <span></span>
                                                                    <p>
                                                                        فوز
                                                                        <span>4</span>
                                                                    </p>
                                                                </div>
                                                            </div>

                                                        </div>
                                                    </div>
                                                    <div>
                                                        <h2 class="tabTitle mx-auto">
                                                            <span>نتائج اخر مباريات بين إمبولي و ساليرنيتانا</span>
                                                        </h2>
                                                        <div class="card">

                                                            <div class="row TeamVsTeam">
                                                                <div class="col dateAndPlace">
                                                                    <p>
                                                                        23.10.2021 <span> الدوري الايطالي </span>
                                                                    </p>
                                                                </div>
                                                                <div class="col teamA">
                                                                    <div class="row lose">
                                                                        <div class="col-7 text-left"><a
                                                                                href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                        </div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/12002.png"
                                                                                alt="ساليرنيتانا"></div>
                                                                        <div class="col-2"><span>2</span></div>
                                                                    </div>
                                                                </div>
                                                                <div class="col teamB">
                                                                    <div class="row win">
                                                                        <div class="col-2"><span>4</span></div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/11867.png"
                                                                                alt="إمبولي"></div>
                                                                        <div class="col-7 text-right"><a
                                                                                href="/team/11867/empoli">إمبولي</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="row TeamVsTeam">
                                                                <div class="col dateAndPlace">
                                                                    <p>
                                                                        07.05.2021 <span> الدرجة الثانية </span>
                                                                    </p>
                                                                </div>
                                                                <div class="col teamA">
                                                                    <div class="row win">
                                                                        <div class="col-7 text-left"><a
                                                                                href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                        </div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/12002.png"
                                                                                alt="ساليرنيتانا"></div>
                                                                        <div class="col-2"><span>2</span></div>
                                                                    </div>
                                                                </div>
                                                                <div class="col teamB">
                                                                    <div class="row lose">
                                                                        <div class="col-2"><span>0</span></div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/11867.png"
                                                                                alt="إمبولي"></div>
                                                                        <div class="col-7 text-right"><a
                                                                                href="/team/11867/empoli">إمبولي</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="row TeamVsTeam">
                                                                <div class="col dateAndPlace">
                                                                    <p>
                                                                        17.01.2021 <span> الدرجة الثانية </span>
                                                                    </p>
                                                                </div>
                                                                <div class="col teamA">
                                                                    <div class="row win">
                                                                        <div class="col-7 text-left"><a
                                                                                href="/team/11867/empoli">إمبولي</a>
                                                                        </div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/11867.png"
                                                                                alt="إمبولي"></div>
                                                                        <div class="col-2"><span>5</span></div>
                                                                    </div>
                                                                </div>
                                                                <div class="col teamB">
                                                                    <div class="row lose">
                                                                        <div class="col-2"><span>0</span></div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/12002.png"
                                                                                alt="ساليرنيتانا"></div>
                                                                        <div class="col-7 text-right"><a
                                                                                href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="row TeamVsTeam">
                                                                <div class="col dateAndPlace">
                                                                    <p>
                                                                        24.07.2020 <span> الدرجة الثانية </span>
                                                                    </p>
                                                                </div>
                                                                <div class="col teamA">
                                                                    <div class="row lose">
                                                                        <div class="col-7 text-left"><a
                                                                                href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                        </div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/12002.png"
                                                                                alt="ساليرنيتانا"></div>
                                                                        <div class="col-2"><span>2</span></div>
                                                                    </div>
                                                                </div>
                                                                <div class="col teamB">
                                                                    <div class="row win">
                                                                        <div class="col-2"><span>4</span></div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/11867.png"
                                                                                alt="إمبولي"></div>
                                                                        <div class="col-7 text-right"><a
                                                                                href="/team/11867/empoli">إمبولي</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="row TeamVsTeam">
                                                                <div class="col dateAndPlace">
                                                                    <p>
                                                                        21.12.2019 <span> الدرجة الثانية </span>
                                                                    </p>
                                                                </div>
                                                                <div class="col teamA">
                                                                    <div class="row win">
                                                                        <div class="col-7 text-left"><a
                                                                                href="/team/11867/empoli">إمبولي</a>
                                                                        </div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/11867.png"
                                                                                alt="إمبولي"></div>
                                                                        <div class="col-2"><span>1</span></div>
                                                                    </div>
                                                                </div>
                                                                <div class="col teamB">
                                                                    <div class="row win">
                                                                        <div class="col-2"><span>1</span></div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/12002.png"
                                                                                alt="ساليرنيتانا"></div>
                                                                        <div class="col-7 text-right"><a
                                                                                href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="row TeamVsTeam">
                                                                <div class="col dateAndPlace">
                                                                    <p>
                                                                        29.03.2018 <span> الدرجة الثانية </span>
                                                                    </p>
                                                                </div>
                                                                <div class="col teamA">
                                                                    <div class="row win">
                                                                        <div class="col-7 text-left"><a
                                                                                href="/team/11867/empoli">إمبولي</a>
                                                                        </div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/11867.png"
                                                                                alt="إمبولي"></div>
                                                                        <div class="col-2"><span>2</span></div>
                                                                    </div>
                                                                </div>
                                                                <div class="col teamB">
                                                                    <div class="row lose">
                                                                        <div class="col-2"><span>0</span></div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/12002.png"
                                                                                alt="ساليرنيتانا"></div>
                                                                        <div class="col-7 text-right"><a
                                                                                href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>

                                                        </div>
                                                    </div>
                                                    <div class="card active">
                                                        <table style="width:100%" class="leagueTable">
                                                            <tbody>
                                                                <tr>
                                                                    <th colspan="5" class="groupTitle"> </th>
                                                                    <th>لعب</th>
                                                                    <th>فوز</th>
                                                                    <th>تعادل</th>
                                                                    <th>خسارة</th>
                                                                    <th>له</th>
                                                                    <th>عليه</th>
                                                                    <th>الفرق</th>
                                                                    <th>النقاط</th>
                                                                </tr>
                                                            </tbody>
                                                            <tbody>
                                                                <tr class="">
                                                                    <td><b>14</b></td>

                                                                    <td colspan="4">
                                                                        <b>
                                                                            <a href="/team/11867/empoli" title="إمبولي">
                                                                                <img src="https://img.btolat.com/teamslogo/11867.png"
                                                                                    alt="إمبولي"
                                                                                    class="img-responsive lazy">
                                                                                إمبولي
                                                                            </a>
                                                                        </b>
                                                                    </td>

                                                                    <td>
                                                                        <div class="color"><span>37</span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>9 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>11 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>17 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>49</span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>70 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>-21 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>38 </span></div>
                                                                    </td>
                                                                </tr>
                                                                <tr class="">
                                                                    <td><b>17</b></td>

                                                                    <td colspan="4">
                                                                        <b>
                                                                            <a href="/team/12002/salernitana"
                                                                                title="ساليرنيتانا">
                                                                                <img src="https://img.btolat.com/teamslogo/12002.png"
                                                                                    alt="ساليرنيتانا"
                                                                                    class="img-responsive lazy">
                                                                                ساليرنيتانا
                                                                            </a>
                                                                        </b>
                                                                    </td>

                                                                    <td>
                                                                        <div class="color"><span>37</span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>7 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>10 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>20 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>33</span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>74 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>-41 </span></div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="color"><span>31 </span></div>
                                                                    </td>
                                                                </tr>
                                                            </tbody>
                                                        </table>
                                                    </div>
                                                    <div>
                                                        <h2 class="tabTitle mx-auto"><span>أكبر فوز</span></h2>
                                                        <div class="card">
                                                            <div class="row TeamVsTeam">
                                                                <div class="col dateAndPlace">
                                                                    <p>07.05.2021 <span> الدرجة الثانية </span></p>
                                                                </div>
                                                                <div class="col teamA">
                                                                    <div class="row win">
                                                                        <div class="col-7 text-left"><a
                                                                                href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                        </div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/12002.png"
                                                                                alt="ساليرنيتانا"></div>
                                                                        <div class="col-2"><span>2</span></div>
                                                                    </div>
                                                                </div>
                                                                <div class="col teamB">
                                                                    <div class="row lose">
                                                                        <div class="col-2"><span>0</span></div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/11867.png"
                                                                                alt="إمبولي"></div>
                                                                        <div class="col-7 text-right"><a
                                                                                href="/team/11867/empoli">إمبولي</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div class="row TeamVsTeam">
                                                                <div class="col dateAndPlace">
                                                                    <p>17.01.2021 <span> الدرجة الثانية </span></p>
                                                                </div>
                                                                <div class="col teamA">
                                                                    <div class="row win">
                                                                        <div class="col-7 text-left"><a
                                                                                href="/team/11867/empoli">إمبولي</a>
                                                                        </div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/11867.png"
                                                                                alt="إمبولي"></div>
                                                                        <div class="col-2"><span>5</span></div>
                                                                    </div>
                                                                </div>
                                                                <div class="col teamB">
                                                                    <div class="row lose">
                                                                        <div class="col-2"><span>0</span></div>
                                                                        <div class="col-2"><img
                                                                                src="https://img.btolat.com/teamslogo/12002.png"
                                                                                alt="ساليرنيتانا"></div>
                                                                        <div class="col-7 text-right"><a
                                                                                href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <h2 class="tabTitle mx-auto">
                                                        <span>نتائج اخر 5 مباريات ل إمبولي</span>
                                                    </h2>
                                                    <div class="card">
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    06.05.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row win">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/11917/internazionale">إنتر
                                                                            ميلان</a>
                                                                    </div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11917.png"
                                                                            alt="إنتر ميلان"></div>
                                                                    <div class="col-2"><span>4</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row lose">
                                                                    <div class="col-2"><span>2</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11867.png"
                                                                            alt="إمبولي"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/11867/empoli">إمبولي</a></div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    01.05.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row lose">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/11867/empoli">إمبولي</a></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11867.png"
                                                                            alt="إمبولي"></div>
                                                                    <div class="col-2"><span>1</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row win">
                                                                    <div class="col-2"><span>3</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12046.png"
                                                                            alt="تورينو"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/12046/torino">تورينو</a></div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    24.04.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row win">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/11867/empoli">إمبولي</a></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11867.png"
                                                                            alt="إمبولي"></div>
                                                                    <div class="col-2"><span>3</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row lose">
                                                                    <div class="col-2"><span>2</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11947.png"
                                                                            alt="نابولي"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/11947/napoli">نابولي</a></div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    16.04.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row win">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/12051/udinese">أودينيزي</a>
                                                                    </div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12051.png"
                                                                            alt="أودينيزي"></div>
                                                                    <div class="col-2"><span>4</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row lose">
                                                                    <div class="col-2"><span>1</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11867.png"
                                                                            alt="إمبولي"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/11867/empoli">إمبولي</a></div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    09.04.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row win">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/11867/empoli">إمبولي</a></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11867.png"
                                                                            alt="إمبولي"></div>
                                                                    <div class="col-2"><span>0</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row win">
                                                                    <div class="col-2"><span>0</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12027.png"
                                                                            alt="سبيزيا"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/12027/spezia">سبيزيا</a></div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <h2 class="tabTitle mx-auto">
                                                        <span> نتائج اخر 5 مباريات ل ساليرنيتانا</span>
                                                    </h2>
                                                    <div class="card">
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    08.05.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row win">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                    </div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12002.png"
                                                                            alt="ساليرنيتانا"></div>
                                                                    <div class="col-2"><span>1</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row win">
                                                                    <div class="col-2"><span>1</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11830.png"
                                                                            alt="كالياري"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/11830/cagliari">كالياري</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    05.05.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row win">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                    </div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12002.png"
                                                                            alt="ساليرنيتانا"></div>
                                                                    <div class="col-2"><span>2</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row lose">
                                                                    <div class="col-2"><span>1</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12088.png"
                                                                            alt="فينيتسيا"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/12088/venezia">فينيتسيا</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    02.05.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row win">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/11811/atalanta">أتالانتا</a>
                                                                    </div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11811.png"
                                                                            alt="أتالانتا"></div>
                                                                    <div class="col-2"><span>1</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row win">
                                                                    <div class="col-2"><span>1</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12002.png"
                                                                            alt="ساليرنيتانا"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    24.04.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row win">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                    </div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12002.png"
                                                                            alt="ساليرنيتانا"></div>
                                                                    <div class="col-2"><span>2</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row lose">
                                                                    <div class="col-2"><span>1</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/11894.png"
                                                                            alt="فيورنتينا"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/11894/fiorentina">فيورنتينا</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="row TeamVsTeam">
                                                            <div class="col dateAndPlace">
                                                                <p>
                                                                    16.04.2022 <span> الدوري الايطالي </span>
                                                                </p>
                                                            </div>
                                                            <div class="col teamA">
                                                                <div class="row lose">
                                                                    <div class="col-7 text-left"><a
                                                                            href="/team/12005/sampdoria">سامبدوريا</a>
                                                                    </div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12005.png"
                                                                            alt="سامبدوريا"></div>
                                                                    <div class="col-2"><span>1</span></div>
                                                                </div>
                                                            </div>
                                                            <div class="col teamB">
                                                                <div class="row win">
                                                                    <div class="col-2"><span>2</span></div>
                                                                    <div class="col-2"><img
                                                                            src="https://img.btolat.com/teamslogo/12002.png"
                                                                            alt="ساليرنيتانا"></div>
                                                                    <div class="col-7 text-right"><a
                                                                            href="/team/12002/salernitana">ساليرنيتانا</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col">
                                                    <br>
                                                    <hr>

                                                    <div class="col">
                                                        <ul class="matchtags">
                                                        </ul>
                                                    </div>



                                                    <br>







                                                </div>


                                            </div>




                                        </div>
                                    </div>
                                    <div class="box fbComm bg-white">
                                        <h2 class="title text-end">التعليقات</h2>
                                        <?php include './comments.php'; ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-4 d-none d-xl-block left-side">
                        <div class="boxes">
                            <div class="box ads">
                                <img src="./assets/images/ads.jpg" alt="" class="img-fluid h-100">
                            </div>
                            <!-- Tabs navs -->
                            <div class="box trend">
                                <div class="outX">
                                    <div class="title">
                                        <div class="more-category" data-section="">
                                            <a>تريند يلاجول</a>
                                        </div>
                                    </div>
                                    <div id="newspointercontent" class="newspointercontent">
                                        <div id="boxUpDown" data-isup="ChartBeatDataUp"
                                            class="post trend news animated slideInUp">
                                            <a href="#" link="www.btolat.com/news/282781" target="_top">

                                                <div class="image">
                                                    <img src="/assets/NewsPointer/images/play-btn.png" class="player">
                                                    <img src="https://img.btolat.com/2022/5/12/news/282781/small.jpg"
                                                        class="img-responsive" alt="2">
                                                    <span>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15">
                                                            <path id="arrow" fill="#09ab24"
                                                                d="M 7.5,0 L 15,15 L 7.5,10.5 L 0,15">
                                                            </path>
                                                        </svg>
                                                    </span>
                                                </div>
                                                <div class="title">
                                                    <div class="top">
                                                        <span class="visitors">198</span>
                                                    </div>
                                                    <h3>فلافيو: أملي في تدريب الأهلي زاد بعد تولي موسيماني.. وتمنيت
                                                        اللعب
                                                        بجوار
                                                        ثنائي الزمالك</h3>
                                                </div>
                                            </a>
                                        </div>
                                        <div id="boxUpDown" data-isup="ChartBeatDataUp"
                                            class="post trend news animated slideInUp">
                                            <a href="#" link="www.btolat.com/news/282781" target="_top">

                                                <div class="image">
                                                    <img src="/assets/NewsPointer/images/play-btn.png" class="player">
                                                    <img src="https://img.btolat.com/2022/5/12/news/282781/small.jpg"
                                                        class="img-responsive" alt="2">
                                                    <span>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15">
                                                            <path id="arrow" fill="#09ab24"
                                                                d="M 7.5,0 L 15,15 L 7.5,10.5 L 0,15">
                                                            </path>
                                                        </svg>
                                                    </span>
                                                </div>
                                                <div class="title">
                                                    <div class="top">
                                                        <span class="visitors">198</span>
                                                    </div>
                                                    <h3>فلافيو: أملي في تدريب الأهلي زاد بعد تولي موسيماني.. وتمنيت
                                                        اللعب
                                                        بجوار
                                                        ثنائي الزمالك</h3>
                                                </div>
                                            </a>
                                        </div>
                                        <div id="boxUpDown" data-isup="ChartBeatDataUp"
                                            class="post trend news animated slideInUp">
                                            <a href="#" link="www.btolat.com/news/282781" target="_top">

                                                <div class="image">
                                                    <img src="/assets/NewsPointer/images/play-btn.png" class="player">
                                                    <img src="https://img.btolat.com/2022/5/12/news/282781/small.jpg"
                                                        class="img-responsive" alt="2">
                                                    <span>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15">
                                                            <path id="arrow" fill="#09ab24"
                                                                d="M 7.5,0 L 15,15 L 7.5,10.5 L 0,15">
                                                            </path>
                                                        </svg>
                                                    </span>
                                                </div>
                                                <div class="title">
                                                    <div class="top">
                                                        <span class="visitors">198</span>
                                                    </div>
                                                    <h3>فلافيو: أملي في تدريب الأهلي زاد بعد تولي موسيماني.. وتمنيت
                                                        اللعب
                                                        بجوار
                                                        ثنائي الزمالك</h3>
                                                </div>
                                            </a>
                                        </div>
                                        <div id="boxUpDown" data-isup="ChartBeatDataUp"
                                            class="post trend news animated slideInUp">
                                            <a href="#" link="www.btolat.com/news/282781" target="_top">

                                                <div class="image">
                                                    <img src="/assets/NewsPointer/images/play-btn.png" class="player">
                                                    <img src="https://img.btolat.com/2022/5/12/news/282781/small.jpg"
                                                        class="img-responsive" alt="2">
                                                    <span>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15">
                                                            <path id="arrow" fill="#09ab24"
                                                                d="M 7.5,0 L 15,15 L 7.5,10.5 L 0,15">
                                                            </path>
                                                        </svg>
                                                    </span>
                                                </div>
                                                <div class="title">
                                                    <div class="top">
                                                        <span class="visitors">198</span>
                                                    </div>
                                                    <h3>فلافيو: أملي في تدريب الأهلي زاد بعد تولي موسيماني.. وتمنيت
                                                        اللعب
                                                        بجوار
                                                        ثنائي الزمالك</h3>
                                                </div>
                                            </a>
                                        </div>
                                        <div id="boxUpDown" data-isup="ChartBeatDataUp"
                                            class="post trend news animated slideInUp">
                                            <a href="#" link="www.btolat.com/news/282781" target="_top">

                                                <div class="image">
                                                    <img src="/assets/NewsPointer/images/play-btn.png" class="player">
                                                    <img src="https://img.btolat.com/2022/5/12/news/282781/small.jpg"
                                                        class="img-responsive" alt="2">
                                                    <span>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15">
                                                            <path id="arrow" fill="#09ab24"
                                                                d="M 7.5,0 L 15,15 L 7.5,10.5 L 0,15">
                                                            </path>
                                                        </svg>
                                                    </span>
                                                </div>
                                                <div class="title">
                                                    <div class="top">
                                                        <span class="visitors">198</span>
                                                    </div>
                                                    <h3>فلافيو: أملي في تدريب الأهلي زاد بعد تولي موسيماني.. وتمنيت
                                                        اللعب
                                                        بجوار
                                                        ثنائي الزمالك</h3>
                                                </div>
                                            </a>
                                        </div>
                                        <div id="boxUpDown" data-isup="ChartBeatDataUp"
                                            class="post trend news animated slideInUp">
                                            <a href="#" link="www.btolat.com/news/282781" target="_top">
                                                <div class="image">
                                                    <img src="/assets/NewsPointer/images/play-btn.png" class="player">
                                                    <img src="https://img.btolat.com/2022/5/12/news/282781/small.jpg"
                                                        class="img-responsive" alt="2">
                                                    <span>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15">
                                                            <path id="arrow" fill="#09ab24"
                                                                d="M 7.5,0 L 15,15 L 7.5,10.5 L 0,15">
                                                            </path>
                                                        </svg>
                                                    </span>
                                                </div>
                                                <div class="title">
                                                    <div class="top">
                                                        <span class="visitors">198</span>
                                                    </div>
                                                    <h3>فلافيو: أملي في تدريب الأهلي زاد بعد تولي موسيماني.. وتمنيت
                                                        اللعب
                                                        بجوار
                                                        ثنائي الزمالك</h3>
                                                </div>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="box ads2">
                                <img src="./assets/images/ads.jpg" alt="" class="img-fluid h-100">
                            </div>
                            <div class="box mostRead">
                                <h2 class="title text-end me-3">الاكثر مشاهدة</h1>
                                    <div class="most">
                                        <span class="number">1</span>
                                        <a href="/news/282752">
                                            <h2>كاف يوضح تفاصيل اختيار ملعب نهائي دوري أبطال إفريقيا.. ويرد على الأهلي
                                            </h2>
                                        </a>
                                        <div class="line w100"></div>
                                    </div>
                                    <div class="most">
                                        <span class="number">2</span>
                                        <a href="/news/282763">
                                            <h2>فرج عامر: مباراة مصر والسنغال ستعاد بنسبة 99% </h2>
                                        </a>
                                        <div class="line w80"></div>
                                    </div>
                                    <div class="most">
                                        <span class="number">3</span>
                                        <a href="/news/282781">
                                            <h2>فلافيو: أملي في تدريب الأهلي زاد بعد تولي موسيماني.. وتمنيت اللعب بجوار
                                                ثنائي
                                                الزمالك </h2>
                                        </a>
                                        <div class="line w60"></div>
                                    </div>
                                    <div class="most">
                                        <span class="number">4</span>
                                        <a href="/news/282755">
                                            <h2>ترتيب الدوري الإنجليزي بعد خماسية مانشستر سيتي أمام ولفرهامبتون </h2>
                                        </a>
                                        <div class="line w40"></div>
                                    </div>
                                    <div class="most">
                                        <span class="number">5</span>
                                        <a href="/news/282757">
                                            <h2>بين بنزيما ومحمد صلاح ومبابي.. جوتي يختار اللاعب الأحق بالكرة الذهبية
                                                2022
                                            </h2>
                                        </a>
                                        <div class="line w20"></div>
                                    </div>
                                    <div class="most">
                                        <span class="number">6</span>
                                        <a href="/news/282773">
                                            <h2>الكومي يناشد الرئيس السيسي بالموافقة على لعب الأهلي في الإسماعيلية </h2>
                                        </a>
                                        <div class="line w0"></div>
                                    </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

</section>
<?php include 'layouts/footer.php'; ?>